...
const years = Array.from({ length: 30 }, (_, i) => 1990 + i); // Generate years from 1990 to current year
...
export function SelectReport({ type, table }: SelectReportProps) {
  ...
  const Component = dynamic(
    () => {
      if (selectedYear && selectedQuarter) {
        if (table)
          return import(
            `@/app/reports/quarterly/${type}/${table}/table/page.tsx`
          );
        return import(`@/app/reports/quarterly/${type}/tables/page.tsx`);
      } else {
